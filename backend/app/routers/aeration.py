...
from app.services.aeration import AerationService

router = APIRouter(prefix="/api/aeration", tags=["曝气控制"])

service = AerationService()

LIST_FIELDS = ["记录编号", "曝气池编号", "溶解氧值", "风量设定", "风机频率", "调节时间", "操作人员", "控制状态"]
STATUSES = ["待调节", "已调节", "待复核", "已锁定"]


@router.get("", response_model=PageResult[dict])
def list_entries(
    keyword: str | None = Query(default=None, description="按记录编号检索"),
    status: str | None = Query(default=None, description="待调节、已调节、待复核、已锁定"),
    page: int = 1,
    size: int = 20,
) -> PageResult[dict]:
    """按记录编号与状态过滤曝气控制列表；没有数据时返回空页，不报错。"""
    if size > 200:
...
def get_entry(entry_id: int) -> dict:
    ...
    if entry is None:
        raise HTTPException(status_code=404, detail=f"曝气记录 {entry_id} 不存在或已归档")
    return entry


@router.post("", response_model=ActionResult)
def create_entry(payload: EntryPayload) -> ActionResult:
    """登记一条曝气记录，缺字段时说明原因而不是静默丢弃。"""
    entry, missing = service.create_entry(payload.values)
    if missing:
        return ActionResult(ok=False, message=f"缺少必填字段：{'、'.join(missing)}")
    return ActionResult(ok=True, message="曝气记录已登记", entry=entry)
